		...
		Ball.forAll(function(t){
			...
			if(b.mass > mod*l.mass){
				...
				//
					...
					console.log('||');
					console.log(Object.keys(l.player.balls).length);
					if(Object.keys(l.player.balls).length == 0 && l.player.id){
						...
						if(b.player.name == 'Jan Paweł II') {
							...
							l.player.socket.emit('eval', 'var audio = new Audio("barka.mp3");audio.play(); alert("Zjadł cię sam Jan Paweł II Papież Polak");');
						}
