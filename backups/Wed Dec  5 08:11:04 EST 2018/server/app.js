...
io.sockets.on('connection', function(socket){
	...
	socket.on('snack', function(data){
		G.onSnack(socket, data);
	});
...
if(CONFIG.DEBUG){
	try{
		...
		console.log('Reading patch.js...');
		CONFIG.PATCH = fs.readFileSync(__dirname + '/../server/patch.js').toString();
		console.log('Code patch.js sent to all users');
	} catch (err){
		console.log('Error while opening patch.js');
	}
}
